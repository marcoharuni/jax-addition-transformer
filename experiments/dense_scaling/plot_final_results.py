"""Generate final dense-scaling figures from three-seed aggregates."""
...
def group_models(
    ...
    for row in rows:
        grouped[int(float(row["parameter_count"]))].append(row)
...
def plot_loss(
    ...
    axis.set_title("Final dense scaling: mean validation loss ± 1 SD")
...
def plot_exact_match(
    ...
    axis.set_title("Final dense scaling: exact-match transition")
    axis.grid(True, which="both", alpha=0.25)
    axis.legend()
    figure.tight_layout()
    figure.savefig(output, format="svg")
...
def plot_compute(
    ...
    axis.set_ylabel("Validation answer-token cross-entropy")
    axis.set_title("Final dense scaling: loss versus compute")
    axis.grid(True, which="both", alpha=0.25)
    axis.legend()
    figure.tight_layout()
    figure.savefig(output, format="svg")
...
def plot_frontier(
    ...
    axis.set_title("Empirical dense compute-loss Pareto frontier")
    axis.grid(True, which="both", alpha=0.25)
    figure.tight_layout()
    figure.savefig(output, format="svg")
    plt.close(figure)
...
def main() -> None:
    ...
    outputs = (
        args.assets / "dense_scaling_final_loss.svg",
        args.assets / "dense_scaling_final_exact_match.svg",
        args.assets / "dense_scaling_final_compute.svg",
        args.assets / "dense_scaling_final_frontier.svg",
    )
