"""Fit and diagnose task-specific matched-active MoE scaling laws."""
...
DEFAULT_OUTPUT = Path(
    "artifacts/moe-scaling-final-t4/final_fit.json"
)
...
def fit_threshold(
    ...
    minimum_loss: float,
    parameter_field: str = "active_parameter_proxy",
) -> dict[str, Any]:
    selected = [
        row
        for row in rows
    ...
    parameters = np.asarray(
        [float(row[parameter_field]) for row in selected],
        dtype=np.float64,
    )
    tokens = np.asarray(
    ...
    for row in ordered:
        ...
        if loss < best_loss:
            frontier.append(
                {
                    "model": row["model"],
                    "active_parameter_proxy": int(float(row["active_parameter_proxy"])),
                    "total_parameter_count": int(float(row["total_parameter_count"])),
                    "dense_reference_parameters": int(float(row["dense_reference_parameters"])),
...
def main() -> None:
    ...
    active_fits = [
        fit_threshold(rows, threshold, "active_parameter_proxy")
        for threshold in THRESHOLDS
    ]
    stored_fits = [
        fit_threshold(rows, threshold, "total_parameter_count")
        for threshold in THRESHOLDS
    ]
    active_stability = threshold_stability(active_fits)
    stored_stability = threshold_stability(stored_fits)
    frontier = empirical_frontier(rows)

    result = {
        "status": active_stability["status"],
        "primary_parameter_axis": "active_parameter_proxy",
        "equation": (
        ...
        "data_definition": (
            ...
            "of 200,000 unique training pairs. The primary N axis is the active "
            "parameter proxy, including routers and top-k selected experts. A "
            "secondary diagnostic repeats the fit with stored total parameters."
        ),
        "group_count": len(rows),
        "seed_count_per_group": 3,
        "active_parameter_fit": {
            "threshold_fits": active_fits,
            "threshold_stability": active_stability,
        },
        "stored_parameter_fit": {
            "threshold_fits": stored_fits,
            "threshold_stability": stored_stability,
        },
        "empirical_frontier_points": len(frontier),
        "conclusion": (
            "Treat active-parameter fits as primary for matched compute and stored-parameter "
            "fits as a capacity diagnostic. Use compute-optimal exponents only when the "
            "corresponding stability status is 'identified'; otherwise report the measured "
            "frontier and the failure of the additive surface to remain stable across cutoffs."
        ),
    }
